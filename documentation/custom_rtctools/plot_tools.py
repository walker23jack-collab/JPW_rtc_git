...
def get_goal_ids_for_priority(priority, current_run):
    """Return goal ids that have the given priority."""
    goal_ids = []

    base_goals = current_run["prio_independent_data"]["base_goals"]

    for goal in base_goals:
        if goal.get("priority") == priority:
            goal_ids.append(goal.get("goal_id"))

    return goal_ids

def get_main_title(final_result: bool, result_dict, current_run=None):
    """Generate main title."""
    if final_result:
    ...
    if current_run is None:
        return f"Results after optimizing until priority {priority}"

    goal_ids = get_goal_ids_for_priority(priority, current_run)
...
def create_matplotlib_figure(
    ...
    main_title = get_main_title(final_result, result_dict, current_run)
...
def set_plotly_layout(plotly_figure, final_result, result_dict, results_compare, current_run=None):
...
def create_plotly_figure(
    ...
    set_plotly_layout(plotly_figure, final_result, result_dict, results_compare, current_run)
